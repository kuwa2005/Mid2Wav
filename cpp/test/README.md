...
```bash
cd cpp
make test              # 全スイート（ユニット 32 + 統合 7 + 品質 7 = 46 ケース）
make test-unit         # SF2 不要: wav + midi + fx（17 ケース）
make test-integration  # 統合スイートのみ（7 ケース）
make test-quality      # 音質・ミックス回帰スイートのみ（7 ケース）
...
## 音質回帰テスト（`quality` スイート、7 ケース）

うねり（振幅変調）と全チャンネル合成経路の回帰を検出します。Review #20/#25 で指摘された v3.1 劣化（per-channel 加算ミックス、chorus ×127）を再発させないことが目的です。

| テスト | 検証内容 | 失敗時の意味 |
|--------|----------|--------------|
| sustained note low amplitude modulation | CC1=0 持続音のデトレンド RMS CV ≤ **0.18** | 持続部に過剰なうねり / ワーブル |
| CC1-heavy sustain vibrato without block-step warble | CC1=80 のデトレンド CV ≤ 0.32、CC1=0 との相関 ≤ 0.995 | ブロック単位 VibLFO による段差うねり、または CC1 未適用 |
